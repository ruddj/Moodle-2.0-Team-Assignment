...
class assignment_team extends assignment_upload {
    ...
    function print_team_admin($team, $filecount, $submission) {
        ...
        $datas = array('team'=>$team);
        $controlform = new control_team_form($action, $datas);
        $controlform ->display();
        ...
        $action = $_SERVER['REQUEST_URI'];
        $datas = array('members'=>$members, 'team'=>$team);
        $removememberform = new remove_team_form($action, $datas);
    ...
    private function print_team_answer($teamid){
        ...
        $submission = $this->get_submission($teammember->student);
        ...
        if ($basedir = $this->team_file_area($teamid)) {
            ...
            if ($this->notes_allowed() and !empty($submission->data1)) {
                $output .= link_to_popup_window ('/mod/assignment/type/upload/notes.php?id='.$this->cm->id.'&amp;userid='.$teammember->student,
                                                'notes'.$teammember->student, get_string('notes', 'assignment'), 500, 780, get_string('notes', 'assignment'), 'none', true, 'notesbutton'.$teammember->student);
                $output .= '&nbsp;';
            }
            $i = 0;
            if ($files = get_directory_list($basedir)) {
...
//                $tiifiles = $this->get_team_tiifiles($teamid);
                ...
                foreach ($files as $key => $file) {
                    ...
                    // Start Optional Turnitin code
//                    if (!empty($tiifiles)) {
//                    	if ( $tiifile = $this->get_tii_file($file, $tiifiles)) {
//                            include_once($CFG->libdir.'/turnitinlib.php');                        
//                            $output .= plagiarism_get_links($tiifile->userid, $file, $this->cm->id, $this->course, $this->assignment);
//                    	}
//                    }
                    // End Optional Turnitin code
                }
...
//                    	if ( $tiifile = $this->get_tii_file($file, $tiifiles)) {
//                            include_once($CFG->libdir.'/turnitinlib.php');                        
//                            $output .= plagiarism_get_links($tiifile->userid, $file, $this->cm->id, $this->course, $this->assignment, true);
//                    	}
//                    }
                    // End Optional Turnitin code
                    $output .= '<br />';
        ...
        if ($this->drafts_tracked() and $this->isopen() and has_capability('mod/assignment:grade', $this->context) and $mode != '') { // we do not want it on view.php page
            if ($this->can_unfinalize($submission)) {
                $options = array ('id'=>$this->cm->id, 'teamid'=>$teamid, 'userrep'=>$userid, 'action'=>'unfinalize', 'mode'=>$mode);
                ...
                $options = array ('id'=>$this->cm->id, 'teamid'=>$teamid, 'userrep'=>$userid, 'action'=>'finalizeclose', 'mode'=>$mode, 'offset'=>$offset);
    ...
    //override the method from upload_assignment
    //use for reversing the team submission.
    function unfinalize() {
    	$teamid = required_param('teamid', PARAM_INT);
        $userrep = required_param('userrep', PARAM_INT);
        $mode   = required_param('mode', PARAM_ALPHA);

        $returnurl = "submissions.php?id={$this->cm->id}&amp;teamid=$teamid&amp;userrep=$userrep&amp;mode=$mode&amp;forcerefresh=1";
    ...
    function finalize() {
        ...
        if ($members && is_array($members)) {
            ...
            if ($team && $this ->is_member($teamid)) {
                $update = new object();
                $update ->id              = $team->id;
                $update -> membershipopen = 0;
                $update ->timemodified    = time();
                update_record('team', $update);
            }
        }
        // Start Optional Turnitin code
//        $plagiarismvalues = get_records_menu('plagiarism_config', 'cm',$this->cm->id,'','name,value');
//        if (!empty($plagiarismvalues['use_turnitin']) &&
//            ($this->drafts_tracked() && isset($this->assignment->tii_draft_submit) && $this->assignment->tii_draft_submit == 1)) {        	
//            include_once($CFG->libdir.'/turnitinlib.php');
//            if ($basedir = $this->team_file_area($teamid)) {
//            	$files = get_directory_list($basedir);
//            }
//            if ($files) {
//            	$tiifile_indb = $this->get_team_tiifiles($teamid);
//            	foreach ($files as $file) {
//            		if ($tii_file = $this->get_tii_file($file, $tiifile_indb)) {
//            			$tii_file->tiicode = 'pending';
//            			$tii_file->tiiscore = '0';
//            			if (!update_record('tii_files', $tii_file)) {
//            				debugging('update tii_files failed');
...
//        } 
        ...
        if(!isset($name) || trim($name)== '' ) {
            ...
            if (get_record('team', 'assignment',$this->assignment->id, 'name',$name)) {
                ...
                if (!isset($userteam)) {
                    ...
                    $createTeam = insert_record('team', addslashes_recursive($team), true) ;
            ...
            if ($insertteam->membershipopen) {
                ...
                if (isset($existmember)) {
                    ...
                    if ($copy) {
                        ...
                        update_record('assignment_submissions', addslashes_recursive($submission));
                    }
                } else {
                    $this -> add_new_team_member($studentid, $teamid);
    ...
    function open_close_team($teamid) {
        ...
        if ($team && $this ->is_member($teamid)) {
            ...
            $team ->timemodified = time();
            update_record('team', addslashes_recursive($team));
        }
    }
    ...
    function submissions($mode) {
        ...
        switch ($mode) {
            ...
            case 'fastgrade':
                ...
                $commenting = false;
                $col        = false;
                $submissioncomment = optional_param('submissioncomment', NULL, PARAM_RAW);
                $menu = optional_param('menu', NULL, PARAM_RAW);
                
                if (isset($submissioncomment)) {
                ...
                foreach (optional_param($col, NULL, PARAM_RAW) as $id => $unusedvalue){
                    ...
                    if ($commenting) {
                        $commentvalue = trim($submissioncomment[$id]);
                        $commentvalue =  clean_text($commentvalue);
                        $commentvalue = addslashes($commentvalue);
            ...
            foreach ($ausers as $auser) {
                ...
                if (!empty($auser->submissionid)) {
                    ...
                    if ($final_grade->locked or $final_grade->overridden) {
                        ...
                        . $auser->id.'" rows="2" cols="20">'.(stripslashes($auser->submissioncomment)).'</textarea></div>';
                    } else {
                        $comment = '<div id="com'.$auser->id.'">'.shorten_text(strip_tags(stripslashes($auser->submissioncomment)),15).'</div>';
                    }
                } else {
                    $studentmodified = '<div id="ts'.$auser->id.'">&nbsp;</div>';
                    $teachermodified = '<div id="tt'.$auser->id.'">&nbsp;</div>';
    ...
    function display_team_marking_window($extra_javascript) {
        ...
        ///End of teacher info row, Start of team info row
        echo '<tr>';
        echo '<td >';
        echo $this ->get_team_link($team);
        echo '</td>';
        echo '<td class="topic">';
        echo '<div class="from">';
        echo '<div class="fullname">'.$team->name.'</div>';
        ...
        echo '</div>';
        $this->print_user_files($userrep, false, $team->id, false);
        echo '</td>';
        echo '</tr>';

        ///End of team info row
    ...
    private function remove_user_from_team($userid , $teamid, $fullright = false) {
        ...
        if ($this->is_member($teamid) || $fullright) {
            ...
            if ($team && !$members) {
                ...
                $updated->timemodified    = time();
                if (!update_record('team', $updated)) {
                	//error_log('print error 2');
                    $this -> print_error();
                } else {
                    $dir = $this->team_file_area_name($teamid);
        ...
        && count($groups)==1) {
            ...
            if (is_array($members) && count($members)>0) {
                foreach ($members as $member) {
                    $userid = $member->student;
                    $user = get_record ('user', 'id', $userid);
                    $selection->addOption(fullname($user));
                }
            }
...
class remove_team_form extends moodleform {
	
	function definition() {
		global $USER, $CFG;
		//error_log('get remove_team_form');
        $mform =& $this->_form;
        $datas = $this->_customdata;
        ...
        $mform ->addElement('submit', 'act_removemember', get_string('removeteammember','assignment_team'));     
        
	}
}

class control_team_form extends moodleform {
	...
	function definition() {
        ...
        $mform ->addElement('submit', 'act_deleteteam', get_string('deleteteam','assignment_team'));     
        $mform ->addElement('submit', 'act_opencloseteam', get_string('openclosemembership','assignment_team'));     
